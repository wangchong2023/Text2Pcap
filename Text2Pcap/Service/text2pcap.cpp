...
void Text2Pcap::ProcessAfterAllComplete() {
    /* 4、删除临时文本文件 */
    QString tmpTextFile = WiresharkUtils::getHex2DumpTextFileName(m_parseInfo.saveFileSuffix, m_parseInfo.dateTime);
...
bool Text2Pcap::isHex2DumpTextFileExist() {
    QString tmpFilePath = WiresharkUtils::getHex2DumpTextFileName(m_parseInfo.saveFileSuffix, m_parseInfo.dateTime);
...
//    quint64 fileSize = tmpFilePath.size();
//    quint64 text2PcapSize = m_parseInfo.text2PcapPacketContent.size();
...
bool Text2Pcap::checkComplete() {
    if (!isParseInfoValid()) {
        QMessageBox::warning(nullptr, UI_RES_CONTROL_MESSAGEBOX_WARNING, INFO_STRING_PARSE_PARAMS_INVALID);
        return true;
    }
    if (isTimerLoopTimesExceed()) {
        QMessageBox::warning(nullptr, UI_RES_CONTROL_MESSAGEBOX_WARNING, INFO_STRING_PARSE_TEXT2PCAP_RETRY_EXCEED);
        return true;
    }
...
bool Text2Pcap::isPcapFileCreateComplete() {
    QString pcapFilePath = WiresharkUtils::getPcapFileName(m_parseInfo.storageFilePath, m_parseInfo.dateTime, m_parseInfo.saveFileSuffix, m_parseInfo.saveFileSuffix);
...
bool Text2Pcap::isTimerLoopTimesExceed() {
    return (m_timerLoopTimes == 0);
}

bool Text2Pcap::isParseInfoValid() {
    return (FileUtils::isFileExist(m_parseInfo.wiresharkPath) && (FileUtils::isFileExist(m_parseInfo.text2PcapPath) && (!m_parseInfo.text2PcapPacketContent.isEmpty())));
}
 ...
 *
 *Example:
 *./text2pcap /Users/constantine/Downloads/hex2dump.txt /Users/constantine/Downloads/test.pcap
*/
bool Text2Pcap::StartText2PcapProcess() {
    QString text2capPath = m_parseInfo.text2PcapPath;

    QString srcHex2DumpFilePath = WiresharkUtils::getHex2DumpTextFileName(m_parseInfo.saveFileSuffix, m_parseInfo.dateTime);
    QString dstSaveFilePath = WiresharkUtils::getPcapFileName(m_parseInfo.storageFilePath, m_parseInfo.dateTime, m_parseInfo.saveFileSuffix, m_parseInfo.saveFileSuffix);
    ...
    if (text2PcapParams.isEmpty()) {
        ...
        argsListText2Pcap << text2PcapParams << srcHex2DumpFilePath << dstSaveFilePath;
    }

    (void)ProcessUtils::StartDetachedProcess(text2capPath, argsListText2Pcap, TIME_WAIT_READY_READ_MSEC);
...
bool Text2Pcap::StartWiresharkProcess() {
    ...
    (void)ProcessUtils::StartDetachedProcess(wiresharkDisplayPath, argsListWireshark, TIME_WAIT_READY_READ_MSEC);
