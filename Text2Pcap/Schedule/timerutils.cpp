...
void TimerUtils::StartTimer(uint32_t timerType) {
  ...
  if (!timerMap.contains(timerType)) {
    qDebug("Start timer(type=%d) failed, type not found.\n", timerType);
    return;
  }

  TIMER_REGISTER_S &registerInfo = timerMap[timerType];
  QTimer *timer = registerInfo.timer;
  if (timer == nullptr) {
    qDebug("Start timer(type=%d) failed, for timer is null.\n", timerType);
    return;
  }
  ...
  /* 增加连接，通过 timerType 从 timerMap 中获取引用，避免悬空指针 */
  connect(timer, &QTimer::timeout, [timerType]() {
    if (timerMap.contains(timerType)) {
      HandleTimeout((void *)(&timerMap[timerType]));
    }
  });
}
