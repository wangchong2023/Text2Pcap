...
QMap<QString, LOG_REGISTER_S *> LogUtils::InitializeLogMap() { return logMap; }
...
void LogUtils::RegisterLog(LOG_REGISTER_S *registerInfo) {
  ...
  if ((registerInfo->filePath.isEmpty()) ||
      (!FileUtils::isFileExist(registerInfo->filePath))) {
    qDebug("Register log failed, for file path is invalid.\n");
    return;
  }

  LOG_REGISTER_S *registerInfoNew = new LOG_REGISTER_S();
...
void LogUtils::log(QtMsgType type, const QMessageLogContext &context,
                   const QString &msg) {
  static QMutex mutex;
  mutex.lock();
  QString text;
  ...
  // 设置输出信息格式
  QString context_info = QString(LOG_OUTPUT_FORMAT_FL)
                             .arg(QString(context.file))
                             .arg(context.line);
  QString strDateTimeDay =
      QDateTime::currentDateTime().toString(TIME_FORMAT_SIMPLE_DAY);
  QString strDateTime =
      QDateTime::currentDateTime().toString(TIME_FORMAT_SIMPLE_MSEC);
  ...
  QFile file(QCoreApplication::applicationDirPath() + LOG_DIR +
             QString(strDateTimeDay.append(FILE_SUFFIX_DOT_LOG)));
