...
bool StringUtils::IsDigit(char ch) { return ((ch >= '0') && (ch <= '9')); }
...
bool StringUtils::IsHex(char ch) { return (IsDigit(ch) || IsHexLetter(ch)); }
...
QString StringUtils::Dec2Number2HexString(uint32_t numberDec) {
  return QString(QSTRING_PARAM_FIRST)
      .arg(numberDec, STRING_FILL_NUM_ZERO_VAL, RADIX_HEX);
}

QString StringUtils::Dec2Number2FilledHexString(uint32_t numberDec,
                                                uint32_t fillWidth) {
  return QString(QSTRING_PARAM_FIRST)
      .arg(numberDec, fillWidth, RADIX_HEX,
           QLatin1Char(STRING_FILL_STR_ZERO_VAL));
}
