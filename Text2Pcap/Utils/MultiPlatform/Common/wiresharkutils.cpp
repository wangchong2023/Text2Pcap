...
bool WiresharkUtils::isWiresharkPathValid(QString displayCustomedWiresharkPath, QString &codeStr) {
...
void WiresharkUtils::writeWiresharkPathConfig(QString displayCustomedWiresharkPath) {    
    QString wiresharkPath = FileUtils::GetSubAppPathByAppAndSubName(displayCustomedWiresharkPath, APP_NAME_WIRESHARK);
    QSettings *setting = new QSettings(getSettingPath(), QSettings::IniFormat);
    setting->setValue(WIRESHARK_CUSTOMED_CONFIG, wiresharkPath);
    setting->setValue(WIRESHARK_CUSTOMED_DISPLAY_CONFIG, displayCustomedWiresharkPath);
    qDebug("Write wiershark path config(%s) to path(%s).", qPrintable(wiresharkPath), qPrintable(setting->fileName()));
    qDebug("Write wiershark display path config(%s) to path(%s).", qPrintable(wiresharkPath), qPrintable(setting->fileName()));
}
...
void WiresharkUtils::writePcapStoragePathConfig(QString pcapStoragePath) {
    ...
    qDebug("Write pcap storage path config(%s) to path(%s).", qPrintable(pcapStoragePath), qPrintable(setting->fileName()));
...
void WiresharkUtils::writePcapFormatConfig(QString pcapFormat) {
    ...
    qDebug("Write pcap format config(%s) to path(%s).", qPrintable(pcapFormat), qPrintable(setting->fileName()));
    qDebug() << setting->fileName();
}

QString WiresharkUtils::getUsingDisplayWiresharkPath(QString customedDisplayWiresharkPath, QString detectedDisplayWiresharkPath) {
    QString displayWiresharkPath = STRING_EMPTY;
    if (!customedDisplayWiresharkPath.isNull() && (FileUtils::isFileExist(customedDisplayWiresharkPath))) {
        displayWiresharkPath = customedDisplayWiresharkPath;
    }
    else if (!detectedDisplayWiresharkPath.isEmpty() && (FileUtils::isFileExist(detectedDisplayWiresharkPath))) {
...
QString WiresharkUtils::geAppPathFromFile(QString appName) {
    ...
    if (appName.compare(APP_NAME_DISPLAY_WIRESHARK) == 0) {
        ...
        appPath = FileUtils::GetSubAppPathByAppAndSubName(usingDisplayWiresharkPath, appName);
    }
...
QString WiresharkUtils::getUsingDisplayWiresharkPath() {
    QSettings *setting = new QSettings(getSettingPath(), QSettings::IniFormat);
    QString customedDisplayWiresharkPath = setting->value(WIRESHARK_CUSTOMED_DISPLAY_CONFIG).toString();
    QString detectedDisplayWiresharkPath =  WiresharkUtils::geAppPath(APP_NAME_WIRESHARK);

   return getUsingDisplayWiresharkPath(customedDisplayWiresharkPath, detectedDisplayWiresharkPath);
}

QString WiresharkUtils::getAppPathByDisplayWiresharkPath(QString appName, QString wiresharkDisplayPath) {
    if (!isAppNameSupport(appName)) {
        return STRING_EMPTY;
    }
...
QString WiresharkUtils::getPcapStoragePath() {
    ...
    QSettings *configSaved = new QSettings(settingPath, QSettings::IniFormat);
...
QString WiresharkUtils::getPcapFormatValue() {
    ...
    QSettings *configSaved = new QSettings(settingPath, QSettings::IniFormat);
    QString pcapFormat = configSaved->value(WIRESHARK_PCAP_Format_CONFIG).toString();

    qDebug("Get pcap format config(%s) from path(%s).", qPrintable(pcapFormat), qPrintable(configSaved->fileName()));
...
bool WiresharkUtils::isAppNameSupport(QString appName) {
    /* 不支持的类型返回空串 */
    if (appName.compare(APP_NAME_WIRESHARK) && appName.compare(APP_NAME_TEXT2CAP) && appName.compare(APP_NAME_DISPLAY_WIRESHARK)) {
        return false;
    }
...
QString WiresharkUtils::getTex2PcapFormatString(QString strPacketContent) {
    ...
    */
    ...
    QString offsetFirst = StringUtils::Dec2Number2FilledHexString(0, WIRESHARK_TEXT2CAP_FILL_WIDTH);
    ...
    for (int i = 0; i < length; i++) {
        ...
        if ((i + 1) % TEXT2CAP_LINE_CHAR_NUM == 0) {
            ...
            /* 每行开始追加偏移值 */
            QString offset = StringUtils::Dec2Number2FilledHexString(((i + 1) / TEXT2CAP_BYTE_TO_CHARS_NUM), WIRESHARK_TEXT2CAP_FILL_WIDTH);
    ...
    /* 最后一行追加偏移值 */
    text2capString.append(StringUtils::Dec2Number2FilledHexString((strPacketContent.length() / TEXT2CAP_BYTE_TO_CHARS_NUM), WIRESHARK_TEXT2CAP_FILL_WIDTH));
...
QString WiresharkUtils::getTex2PcapFormatStringNew(QString strPacketContent) {
    ...
    */
    ...
    for (row = 0; row < byteArrSize; row += colNumFix) {
        dataStream << std::setw(WIRESHARK_TEXT2CAP_FILL_WIDTH) << std::setfill(STRING_CHAR_ZERO) << row << STRING_CHAR_BLANK;
        for (col = 0; col < colNumFix; col++)
        {
            if (row + col < byteArrSize) {
                dataStream << std::setw(2) << std::setfill(STRING_CHAR_ZERO) << (int)byteArr[row + col] << STRING_CHAR_BLANK;
            }
        }
...
*/
QString WiresharkUtils::getParseFileName(QString rootPath, QDateTime curDateTime, QString prefix, QString suffix) {
    QString curDateTimeString = curDateTime.toString(TIME_FORMAT_SIMPLE_MSEC_UNDERLINE);
    QString filePath = rootPath + QDir::separator() + prefix + FILE_NAME_PREFIX + FILE_SEP_UNDERLINE + curDateTimeString + FILE_SUFFIX_DOT + suffix;
    QString fileCanonicalPath = QFileInfo(filePath).absoluteFilePath();

    return fileCanonicalPath;
}

QString WiresharkUtils::getHex2DumpTextFileName(QString prefix, QDateTime curDateTime) {
    return getParseFileName(QDir::tempPath() , curDateTime, prefix, FILE_EXTENTION_TXT);
}

QString WiresharkUtils::getPcapFileName(QString rootPath, QDateTime curDateTime, QString prefix, QString suffix) {
...
QString WiresharkUtils::getSettingPath() {
    QString path = QDir::toNativeSeparators(QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) + "/" + CONFIG_FILE_NAME);
    return path;
}
